# StayOnBeat AI 代理协作指南

> 用途：定义 AI 代理在仓库中的工作方式、验收标准和决策记录。
> 优先级：低于 `AGENTS.md`，高于代理的默认行为。

## 1. 角色定位

AI 代理在本仓库承担三类工作：

1. **方案整理**：整理需求、竞品参考、技术选项和风险。
2. **实现**：在文档基线批准后，实现功能并保持与文档一致。
3. **验证**：运行测试、检查行为、报告偏差。

当前仓库已启动实现，M1–M5（节拍器核心/设置与体验/训练评分/总结与历史/发布准备）已完成；MVP 待部署上线。

## 2. 任务启动清单

开始任何任务前，先确认：

- [ ] 已读 `AGENTS.md`。
- [ ] 已读与本任务相关的产品/技术文档。
- [ ] 已读 `docs/development-plan.md` 的里程碑与任务拆解。
- [ ] 明确“本次只产出文档”还是“可以写代码”。
- [ ] 识别需要修改的文档文件，避免跨领域误改。
- [ ] 如任务会改变架构或评分规则，先记录到决策记录。

## 3. 常见任务模板

### 3.1 新增功能

1. 在 `docs/product-design.md` 补充用户价值、流程、交互、验收指标。
2. 在 `docs/technical-solution.md` 补充模块、数据结构、边界情况、测试策略。
3. 如涉及视觉，补充布局、颜色、动效、可访问性说明。
4. 用户批准后才进入实现。

### 3.2 修改判定/评分

1. 同步修改产品设计“判定与评分 UX”。
2. 同步修改技术方案“匹配度与判定算法”和“数据模型”。
3. 更新测试策略。
4. 在决策记录说明理由。

### 3.3 新增依赖

1. 在技术选型表中写明库、用途、替代方案。
2. 说明包体积、维护风险、是否引入后端能力。
3. 用户确认后再安装。

## 4. 验收清单（未来编码阶段）

功能完成前至少确认：

- [ ] 节拍调度不依赖业务定时器发声。
- [ ] 键盘重复与多输入源重复不会重复计分。
- [ ] 静音模式下视觉节拍清晰可用。
- [ ] 刷新页面后设置可恢复，训练记录可保存。
- [ ] 训练中修改配置不会导致评分错乱。
- [ ] 判定/匹配度有单元测试覆盖。
- [ ] 暗色/亮色主题均可用。
- [ ] 移动端触控点击无 300ms 级明显延迟。
- [ ] 无障碍：可键盘操作、焦点可见、动画可关闭。

## 5. 评审关注点

- 是否改变产品定位或 MVP 范围。
- 是否增加不必要的复杂度。
- 是否复制参考站点素材或文案。
- 是否新增未批准的后端/云服务。
- 是否把计时精度问题掩盖在视觉层。
- 是否导致移动端或静音模式体验劣化。

## 6. 决策记录

| 日期 | 决策 | 理由 | 状态 |
| --- | --- | --- | --- |
| 2026-08-21 | 项目先产出产品设计、技术方案、AGENTS.md，暂不编码 | 用户明确要求文档先行 | 生效 |
| 2026-08-21 | 建议 React + Vite + TS + Tailwind + Zustand | 交互面板复杂但无后端，重前端场景 | 生效 |
| 2026-08-21 | MVP 判定窗口先以 40/80/120ms 为名义上限 | 参考节奏游戏体验；下一条已细化为动态窗口 | 已细化 |
| 2026-08-21 | 判定窗口改为随 BPM/细分动态收紧 | 消除 Good ≤ 120ms 与 150ms 有效窗口的边界冲突 | 生效 |
| 2026-08-21 | 默认暗色、突出静音/视觉节拍 | 办公室低打扰是核心场景 | 生效 |
| 2026-08-21 | 增加“节拍器 / 训练”双模式，默认训练 | 保留参考站基础节拍器能力，训练计分作为核心差异化 | 生效 |
| 2026-08-21 | 首次进入默认静音，开启声音后音量 0.5 | 避免办公室突发声音，同时保留合理默认音量 | 生效 |
| 2026-08-21 | 参考站仅作交互参考，禁止复制素材 | 避免版权与品牌混淆 | 强制 |
| 2026-08-21 | 新增 `CLAUDE.md`，仅作 Claude Code 定向入口 | 与 AGENTS.md 互补，避免重复内容 | 生效 |
| 2026-08-21 | 参考站「速度术语显示」保留并纳入 MVP；「440Hz 校音音」「多音色」不纳入 MVP | 补齐参考站功能清单，聚焦跟拍训练 | 生效 |
| 2026-08-21 | M1 细化：lookahead 调度循环归 `MetronomeEngine`，`AudioEngine` 退化为纯发声层 | 节拍序列可复用于 M3 评分，职责单一 | 生效 |
| 2026-08-21 | `ScheduledBeat.stop()` 用 `disconnect()` 取消未启动节拍 | Web Audio 中未 `start()` 的节点调 `stop()` 抛 `InvalidStateError`，需断开连线才能真正撤销未来排定的发声 | 生效 |
| 2026-08-21 | M1 视觉相位直接读调度同源时钟 `ctx.currentTime`，不经 `getOutputTimestamp` 桥接 | `AudioTimestamp.performanceTime` 跨浏览器时间基不一致会引入任意相位偏移（拍灯从第 2/3 拍闪、重音对不上） | 生效 |
| 2026-08-21 | M2：`beatIndexAtAudioTime` 保持拍内序号，另增 `subdivisionIndexAtAudioTime` | 拍灯表达拍号结构、细分是每拍密度，两维度分开喂视觉；M1 契约与既有测试不变 | 生效 |
| 2026-08-21 | M2：静音仍建 AudioContext 驱动时钟，`scheduleBeat` 静音时返回空句柄不创建节点 | 视觉读 `ctx.currentTime`，无运行时钟则视觉冻结；运行中无源 AudioContext 不产生可听输出 | 生效 |
| 2026-08-21 | M2：主题用 CSS 变量 + `data-theme`，不用 Tailwind `dark:` 变体 | 现有组件硬编码暗色，CSS 变量把色板集中、切换只翻转一个属性，迁移为纯机械替换 | 生效 |
| 2026-08-21 | M2：设置持久化用 zustand `persist`（key `stayonbeat-settings` v1），测试工厂默认关闭 | 声明式 partialize + 自动 rehydrate；测试默认不触碰 localStorage | 生效 |
| 2026-08-21 | M2：计时器归 `MetronomeEngine` 音频时钟驱动，`setOnStopped` 回调，到点不 flush 不 suspend | store 级 `setTimeout` 后台会节流且与调度不同源；到点让窗口内最后几拍自然播完 | 生效 |
| 2026-08-21 | M2：Tap BPM 取中位数抗抖动，过滤 200–3000ms，间隔 >2500ms 重置 | 连点/停顿会把均值带偏 | 生效 |
| 2026-08-21 | M3：`mode`/`countInEnabled` 持久化于 metronome store，训练运行时独立 `useTrainingStore` | 设置与运行时分离，避免评分会话状态污染持久化 | 生效 |
| 2026-08-21 | M3：输入采集用 hook，去重/时间基换算抽 `src/lib/input.ts` 纯函数 | 时间基判别与去重窗口可单测；hook 只管监听 | 生效 |
| 2026-08-21 | M3：Miss 过期由 50ms tick 驱动（`expireMissedBeats`），非查询触发 | 用户停止点击时也能结算 Miss，避免停滞 | 生效 |
| 2026-08-21 | M3：`addOnStopped` 与 `setOnStopped` 并存，训练经 `addOnStopped` 收计时器到点 | 训练要收「到点→completed」，与手动停止区分 | 生效 |
| 2026-08-21 | M3：后台切回按 `aborted` 结算；训练模式跳过 `resumeAfterBackground` | 避免节拍时间轴重排破坏评分 | 生效 |
| 2026-08-21 | M3：预期拍时间确定性计算 `expectedBeatTime(k)=firstScoringTime+k*spSub` | 评分只需 `getFirstBeatTime()`+配置，不必暴露排拍序列 | 生效 |
| 2026-08-21 | M3：persist version 保持 1，新增字段浅合并回退默认 | 避免旧 localStorage 数据因版本升级失效 | 生效 |
| 2026-08-21 | M3 输入评分用输入时钟桥（不依赖 `getOutputTimestamp`），与视觉/调度同源 | `getOutputTimestamp().performanceTime` 跨浏览器时间基不一致会引入大偏移，导致训练输入无法命中预期拍 | 生效 |
| 2026-08-21 | 训练视觉由 training phase 驱动（`shouldRunBeatPulse`），非 `isPlaying`；后台 hidden 中止训练 | 训练模式不置 metronome isPlaying，rAF 需按 phase 运行；后台中止避免节拍时间轴重排破坏评分 | 生效 |
| 2026-08-21 | M4：历史持久化用原生 IndexedDB 薄适配 + 可注入存储，不新增依赖 | 训练 store 不直接碰 IndexedDB；测试用 memory storage 覆盖 save/list/clear | 生效 |
| 2026-08-21 | M4：保存时机在 summary 侧 `useSaveSessionToHistory`（按 result 引用去重） | 训练 store 保持纯；一次会话只保存一次 | 生效 |
| 2026-08-21 | M4：会话记录补 `id/startedAt/endedAt` | M3 `SessionResult` 缺三字段；补全后与 §8.2 数据模型一致 | 生效 |
| 2026-08-21 | M5：部署 Vercel 或 Cloudflare Pages 二选一（静态、零后端、无环境变量） | MVP 纯前端，无服务端依赖 | 生效 |
| 2026-08-21 | M5：CI 用 GitHub Actions（typecheck/lint/unit/build，E2E 可选） | 仓库在 GitHub；轻量且免费 | 生效 |
| 2026-08-21 | M5：性能优化克制，仅懒加载 + 体积审视 | 不引入重型优化；NFR 用 DevTools 实测 | 生效 |
| 2026-08-21 | M5：E2E 汇总为单一可跑套件 | 统一验证核心闭环，浏览器就绪后执行 | 生效 |
| 2026-08-21 | M5：可访问性（键盘全流程/焦点/reduced-motion）为发布硬性验收 | 核心场景依赖键盘与动画可关闭 | 生效 |

## 7. 输出要求

AI 代理最终回复应至少说明：

- 完成了什么。
- 修改/新增了哪些文件。
- 是否影响产品、技术或评分规则。
- 哪些内容需要用户确认。
- 下一步建议。

若没有修改文件，直接说明分析结论和下一步即可。
